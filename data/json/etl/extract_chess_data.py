...
def extract_chess_player_data(username:str, number_month:int=3):
    print(f"--- Début de l'extraction des données pour {username} ---")
    archives=get_chess_data(username)
    if not archives:
        print(f"No data found with {username}, play chess.")
        return 0
    ...
    for i, archive_url in enumerate(archives[-number_month:]):
        ...
        time.sleep(1)
    print(f"--- Extraction terminée. {total_games_downloaded} parties téléchargées pour {username}. ---")
